...
public class SearchCrawler implements Runnable {
	...
	public ArrayList<String> crawl(String startUrl, int maxUrls, String searchString, boolean limithost, boolean caseSensitive) {
		...
		while (toCrawlList.size() > 0) {
			...
			if (pageContents != null && pageContents.length() > 0) {
				...
				if (iCount > 10) {
					System.out.println(new SimpleDateFormat("yyyy年MM月dd日HH时mm分ss秒").format(new Date(Calendar.getInstance().getTimeInMillis())) + "=====>" + "超过10张...");
					try {
						Thread.sleep(2000);
					} catch (InterruptedException e) {
						// TODO Auto-generated catch block
	...
	public class ImgDownThread implements Runnable {
		...
		public void run() {
			try {
				...
				in.close();
				System.out.println(new SimpleDateFormat("yyyy年MM月dd日HH时mm分ss秒").format(new Date(Calendar.getInstance().getTimeInMillis())) + "=====>" + "下载了图片" + stru);
	...
	public class TimeWrite2File implements Runnable {
		public void run() {
			while (blnFlag) {
				try {
					System.out.println(new SimpleDateFormat("yyyy年MM月dd日HH时mm分ss秒").format(new Date(Calendar.getInstance().getTimeInMillis())) + "=====>" + "序列化URL...");
